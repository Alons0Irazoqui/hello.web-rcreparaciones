/* ═══════════════════════════════════════════════════════════
   RC REFRIGERACIÓN — main.js
   Paleta: Azul RC + Frío + Industrial
   ═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
   2. HERO CANVAS — Gotas de agua + Líneas de velocidad
      (motivos visuales del logo de Reparaciones MG)
   ═══════════════════════════════════════════════════════════ */
function initHeroCanvas() {
  ...
  // Paleta RC Refrigeración — tonos fríos/cyan
...
function initStaggerObserver() {
  ['.services-grid', '.benefits-grid', '.process-wrap'].forEach(sel => {
...
function initContactForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    const equipoLabel = {
      ...
      restaurante:   'Equipo de restaurante',
      refrigerador:  'Refrigerador',
      congelador:    'Congelador industrial',
    ...
    const text = encodeURIComponent(
      ...
      `📱 Tel: ${telefono}\n` +
      `❄️ Equipo: ${equipoLabel}\n` +
      `🏢 Negocio/Ubicación: ${colonia}\n` +
      (mensaje ? `💬 Problema: ${mensaje}` : '')
    );

    window.open(`https://wa.me/526674134813?text=${text}`, '_blank');
...
function animateCounters() {
  ...
  const counters = [
    { target: 300,  suffix: '+',      prefix: '+' },
    { target: 10,   suffix: '+ años', prefix: '' },
    { target: 98,   suffix: '%',      prefix: '' },
    { target: 100,  suffix: '%',      prefix: '' },
  ];

  statNums.forEach((el, i) => {
    if (!counters[i]) return;
    const { target, suffix, prefix } = counters[i];
    const obj = { val: 0 };
